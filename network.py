...
def _build_model(network, params):
    ...
    if params.ess_relax:
        model.shared_es_penalty = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0)
        ...
        for s_m in model.scenarios_market:
            for s_o in model.scenarios_operation:
                for p in model.periods:
                    ...
                    # ESS operation
                    model.shared_energy_storage_operation.add(sch <= s_max + SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(sdch <= s_max + SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(pch <= s_max + SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(pdch <= s_max + SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(qch <= tan(max_phi) * pch + SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(qch >= tan(min_phi) * pch - SMALL_TOLERANCE)
                    model.shared_energy_storage_operation.add(qdch <= tan(max_phi) * pdch + SMALL_TOLERANCE)
                    ...
                    # Charging/discharging complementarity constraints
                    if params.ess_relax:
                        model.shared_energy_storage_ch_dch_exclusion.add(sch * sdch <= model.shared_es_penalty[e, s_m, s_o, p])
                    else:
                        # NLP formulation
    ...
    if network.is_transmission:
        for dn in model.active_distribution_networks:
            ...
            for p in model.periods:
                ...
                if not params.interface_relax:
                    model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[dn, p] - expected_vmag_sqr >= -SMALL_TOLERANCE)
                    model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[dn, p] - expected_vmag_sqr <= SMALL_TOLERANCE)
                else:
                    model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[dn, p] - expected_vmag_sqr <= model.penalty_expected_interface_vmag_sqr_up[dn, p])
                    model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[dn, p] - expected_vmag_sqr >= model.penalty_expected_interface_vmag_sqr_down[dn, p])
                '''
        ...
        for p in model.periods:
            ...
            if not params.interface_relax:
                ...
                model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[p] - expected_vmag_sqr <= model.penalty_expected_interface_vmag_sqr_up[p])
                model.expected_interface_voltage.add(model.expected_interface_vmag_sqr[p] - expected_vmag_sqr <= model.penalty_expected_interface_vmag_sqr_down[p])
    ...
    if params.obj_type == OBJ_MIN_COST:
        ...
        if params.interface_relax:
            if network.is_transmission:
                for dn in model.active_distribution_networks:
                    for p in model.periods:
                        obj += PENALTY_INTERFACE * (model.penalty_expected_interface_vmag_sqr_up[dn, p])
                        obj += PENALTY_INTERFACE * (model.penalty_expected_interface_vmag_sqr_down[dn, p])
            else:
                for p in model.periods:
                    obj += PENALTY_INTERFACE * (model.penalty_expected_interface_vmag_sqr_up[p])
                    obj += PENALTY_INTERFACE * (model.penalty_expected_interface_vmag_sqr_down[p])
            ...
            if params.slacks_used:
                ...
                if params.ess_relax:
                    for e in model.shared_energy_storages:
                        ...
                        for p in model.periods:
                            slack_comp = pe.value(model.shared_es_penalty[e, s_m, s_o, p])
